...
uint16_t UARTDevice::getAvailDataSize() { return circular_buf_->getNumAvailBytes(); }

bool UARTDevice::read(uint8_t* buf, uint16_t size) { return circular_buf_->read(buf, size); }