...
size_t MavlinkEncoder::packIntoMavlinkByteArray(IncomingData &data, uint8_t outputBuffer[],
    ...
    if (data.isLatitudeInitialized || data.isLongitudeInitialized || data.isAltitudeInitialized) {
        ...
        tempSize =
            ENCODE_MESSAGE(global_position_int, 0, data.latitude, data.longitude, data.altitude, 0,
